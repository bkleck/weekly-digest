...
# make use of no. of words to separate dataframes for different summarizations
df['words'] = df['article'].apply(lambda x: word_tokenize(x))
df['length'] = df['words'].apply(lambda x: len(x))
small_df = df[df['length'] <= 200]
big_df = df[df['length'] > 200]
...
# load model from 'summary_weights' folder instead due to pre-trained weights
model = T5ForConditionalGeneration.from_pretrained('summary_weights')
...
def short_summarize(text):
  input_ids = tokenizer.encode(text, return_tensors="pt", add_special_tokens=True, max_length=512, truncation=True)
  input_ids = input_ids.to(device) # ensure inputs and model on same device

  generated_ids = model.generate(input_ids=input_ids, 
                                #  min_length=50, 
                                 max_length=150, 
                                 length_penalty=1.0, # set penalty for increasing length (> 1 means force to increase length)
...
small_df['summary'] = small_df['article'].apply(lambda x: short_summarize(x))
big_df['summary'] = big_df['article'].apply(lambda x: long_summarize(x))
df = pd.concat([small_df, big_df])
df.drop(columns=['words', 'length'], inplace=True)
